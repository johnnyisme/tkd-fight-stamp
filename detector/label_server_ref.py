#!/usr/bin/env python3
"""
極簡裁判標記工具（零額外依賴，只用 Python 內建 http.server）。

用途：逐張顯示訓練幀，你用滑鼠拖一個框框住裁判，存成 YOLO 格式標記。
  - 一張圖只標一個框（裁判），class id = 0
  - 標記檔輸出到 dataset/labels/<same_name>.txt，格式: "0 xc yc w h"（皆正規化 0..1）
  - 裁判被完全遮擋看不到 → 按「跳過(無裁判)」，寫入空標記檔（負樣本）
...
IMG_DIR = os.path.join(HERE, "dataset_ref", "images")
LBL_DIR = os.path.join(HERE, "dataset_ref", "labels")
os.makedirs(LBL_DIR, exist_ok=True)

IMAGES = sorted(f for f in os.listdir(IMG_DIR) if f.lower().endswith((".jpg", ".png")))

PAGE = """<!DOCTYPE html>
<html lang="zh-Hant"><head><meta charset="utf-8"><title>裁判標記</title>
<style>
...
<header>
  ...
  <button class="warn" onclick="skip()">跳過·無裁判 (X)</button>
  <span class="hint">框住場上那位「裁判」（穿襯衫/領帶、站在兩位選手之間或場邊，不穿護具）。框住整個人（頭到腳）。裁判被完全擋住看不到就按「跳過」。</span>
</header>
...
async function save(){
  const nb=normBox();
  if(!nb){ alert('請先拖一個框框住裁判，或按「跳過」'); return; }
